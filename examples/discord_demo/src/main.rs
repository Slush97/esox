...
use esox_ui::{
    ClipboardProvider, InputState, Rect, RichText, SpacingScale, Span, Status, TextRenderer, Theme,
    Ui, UiState, id,
};
        ...
        Self {
            ...
            members: vec![
                Member {
                    ...
                    color: Color::new(0.38, 0.77, 0.55, 1.0),
                },
                Member {
                    name: "bob",
                    status: Status::Online,
                    color: Color::new(0.55, 0.63, 0.82, 1.0),
                },
                Member {
                    name: "charlie",
                    status: Status::Idle,
                    color: Color::new(0.85, 0.65, 0.30, 1.0),
                },
                Member {
                    name: "diana",
                    status: Status::Offline,
                    color: Color::new(0.60, 0.60, 0.60, 1.0),
                },
                Member {
                    name: "eve",
                    status: Status::Online,
                    color: Color::new(0.82, 0.45, 0.45, 1.0),
                },
            ],
            messages: vec![
                Message {
                    sender: "alice",
                    content: "hey everyone! just pushed the new sidebar widget",
                    timestamp: "Today at 11:32 AM",
                    show_sender: true,
                },
                Message {
                    sender: "alice",
                    content: "it supports header, scrollable body, and footer zones",
                    timestamp: "Today at 11:32 AM",
                    show_sender: false,
                },
                Message {
                    sender: "bob",
                    content: "nice! does it handle the selected state for items?",
                    timestamp: "Today at 11:34 AM",
                    show_sender: true,
                },
                Message {
                    sender: "alice",
                    content: "yep, .selected(true) gives it the accent background, and there's hover animations too",
                    timestamp: "Today at 11:35 AM",
                    show_sender: true,
                },
                Message {
                    sender: "charlie",
                    content: "what about badges for unread counts?",
                    timestamp: "Today at 11:38 AM",
                    show_sender: true,
                },
                Message {
                    sender: "alice",
                    content: ".badge(count) on the item builder, renders a red pill on the right",
                    timestamp: "Today at 11:39 AM",
                    show_sender: true,
                },
                Message {
                    sender: "eve",
                    content: "tested it with veil-client, works great with split_pane_h_mut",
                    timestamp: "Today at 11:42 AM",
                    show_sender: true,
                },
                Message {
                    sender: "bob",
                    content: "the fractional glyph positioning experiment was interesting",
                    timestamp: "Today at 11:45 AM",
                    show_sender: true,
                },
                Message {
                    sender: "bob",
                    content: "but bilinear filtering on bitmap glyphs just makes everything blurry",
                    timestamp: "Today at 11:45 AM",
                    show_sender: false,
                },
                Message {
                    sender: "charlie",
                    content: "yeah you need SDF text or sub-pixel offset rasterization for that to work",
                    timestamp: "Today at 11:47 AM",
                    show_sender: true,
                },
                Message {
                    sender: "diana",
                    content: "i'll look into MSDF text rendering next week",
                    timestamp: "Today at 12:01 PM",
                    show_sender: true,
                },
                Message {
                    sender: "alice",
                    content: "sounds good. for now the integer-rounded positions with nearest sampling are crisp enough",
...
impl AppDelegate for DiscordApp {
    ...
    fn on_redraw(
        ...
        let member_data: Vec<(&str, Status, Color)> = self
            .members
            .iter()
            .map(|m| (m.name, m.status, m.color))
            .collect();
        let messages: Vec<(&str, &str, &str, bool)> = self
            .messages
            .iter()
            .map(|m| (m.sender, m.content, m.timestamp, m.show_sender))
            .collect();
            ...
            match panel {
                            ...
                            for &(name, status, color) in &member_data {
                                ui.padding(SpacingScale::Lg, |ui| {
                                    ui.row_spaced(ui.theme().content_spacing, |ui| {
                                        ...
                                        let c = if matches!(status, Status::Offline) {
                                            ui.theme().fg_dim
                                        } else {
                                            ui.theme().fg
                                        };
                ...
                1 => {
                    ...
                    ui.surface(|ui| {
                        ui.padding(SpacingScale::Lg, |ui| {
                            ui.row(|ui| {
                                ...
                                let online = member_data
                                    .iter()
                                    .filter(|(_, s, _)| !matches!(s, Status::Offline))
                                    .count();
                    ...
                    ui.scrollable_fill(id!("msg_scroll"), |ui| {
                        ui.padding(SpacingScale::Lg, |ui| {
                            for (i, &(sender, content, timestamp, show_sender)) in
                                messages.iter().enumerate()
                            {
                                if show_sender {
                                    if i > 0 {
                                        ui.add_space(ui.theme().content_spacing * 1.5);
                                    }
                                    let name_color = member_data
                                        .iter()
                                        .find(|(n, _, _)| *n == sender)
                                        .map(|(_, _, c)| *c)
                                        .unwrap_or(ui.theme().fg);
                                    ui.row_spaced(ui.theme().content_spacing, |ui| {
                                        ui.avatar_colored(&sender[..2], 32.0, name_color);
                                        let dim = ui.theme().fg_dim;
                                        ui.rich_label(
                                            &RichText::new().colored_bold(sender, name_color).push(
                                                Span {
    ...
    fn cursor_icon(&self, x: f64, y: f64) -> esox_input::CursorIcon {
